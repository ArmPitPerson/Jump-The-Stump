...
void AMainGame::Render() {
	...
	PreRenderedImage.update(GameWindow);
	sf::Sprite PS(PreRenderedImage);

	ScanlineFragShader.setUniform("currentTime", TimeSinceStart.getElapsedTime().asSeconds());

	GameWindow.draw(PS, &ScanlineFragShader);
